...
abstract class dbBaseClass
{
     ...
     *                                        <pre> NOTE! If the value is an array then it must be in the
     *                                        follwing format: array('>', 10) is understood as: `column > 10`
     *                                        array('<=', '10') is understood as: `column <= '10'` etc...
     *                                        </pre> For the 'IN' operator the value will be treated as a
     *                                        simple CSV (separates made of a comma only) or an array. <pre>
     *                                        Example (both have same effect): array('IN', '1,2,3')
     *                                        array('IN', array(1,2,3)) NOTE! In both cases the values are
     *                                        preparaed acording to standard rules (i.e. escaped or cast to
     *                                        int). </pre> Characters allowed for operators are: [<>=!] and
     *                                        [RLIKE], IN, NOT IN, IS, IS NOT (last to will ignore value and
     *                                        assume it to be NULL)
     *
     ...
     *                                        When false it's like setting "LIKE" as a default operator
     *                                        (otherwise equality is default).
     ...
     *                               Note! Constraints can be used only if a marker is added in the template: "{pv_constraints}" or
     *                               "{pv_constraints|default_constraint=1}" Typically the marker would appear in the WHERE clause. Example marker: "SELECT * FROM
     *                               jobs WHERE {pv_constraints|active=1}" So by default this results in: "SELECT * FROM jobs WHERE active=1" With constraints like
     *                               so: array('type'=>'builder'); it would become: "SELECT * FROM jobs WHERE type='builder'"
     *
     ...
     *                                        values (columns) that:
     *                                        a) can be modified b)
     *                                        are to be modified If
     *                                        you must you can also
     ...
     *                                        Only those defined for the record are available. If
     *                                        empty all of them are retrived.
     *
     * @param  bool  $pv_areConstraintsPrecise [optional] Use instead of adding LIKE operator for every value.
     * @return int 0 upon error or no records matched.
     * @throws Exception If \a pv_tableName was not set.
