...
<div class="group-stats-container">
    ...
    <?php foreach (dbProfile::$pv_grupy as $g) { ?>
    <?php foreach ($tplData['grupy_liczniki'] as $row) { ?>
                <?php if ($g == $row['nazwa']) { ?>
                    <li><?php echo $row['nazwa']?> (<?php echo $row['licznik']?>)</li>
                <?php } ?>
    <?php } ?>
    <?php } ?>
    </ul>
...
<form id="search" method="post" action="" class="draw-history-profile-form">
    ...
    <div>
        ...
        <div>
            <select name="miejsce">
                ...
                <?php foreach ($tplData['miejsce'] as $row) { ?>
                    <option <?php echo ($tplData['prev']['miejsce']==$row['miejsce']) ? 'selected' : ''?>
                        value="<?php echo $row['miejsce']?>"><?php echo $row['miejsce']?> (<?php echo $row['licznik']?>)</option>
                <?php } ?>
            </select>
        </div>
    </div>
    <div>
        ...
        <div>
            od: <input type="number" name="wiek_od" min="18" max="100"
                       value="<?php echo $tplData['prev']['wiek_od']?>"
                       >
            do: <input type="number" name="wiek_do" min="18" max="100"
                       value="<?php echo $tplData['prev']['wiek_do']?>"
                       >
    ...
    <div>
        ...
        <div class="buttonset">
    <?php foreach ($tplData['wyksztalcenie'] as $i=>$row) { ?>
            <input id="wyksztalcenie_<?php echo $i?>" type="radio" name="wyksztalcenie" value="<?php echo $row['wyksztalcenie']?>"
                        data-longValue="<?php echo dbProfile::pf_wyksztalcenieTranslate($row['wyksztalcenie'])?>"
        <?php echo ($row['wyksztalcenie'] == $tplData['prev']['wyksztalcenie']) ? 'checked' : ''?>
                    >
            <label for="wyksztalcenie_<?php echo $i?>"><?php echo dbProfile::pf_wyksztalcenieTranslate($row['wyksztalcenie'])?>
                (<?php echo $row['licznik']?>)</label>
    <?php } ?>
            <input id="wyksztalcenie_i" type="radio" name="wyksztalcenie" value=""
                        <?php echo empty($tplData['prev']['wyksztalcenie']) ? 'checked' : ''?>
                   >
...
if (!empty($tplData['profiles'])) {
    ...
    ModuleTemplate::printArray(
        $tplData['profiles'], array(
...
<script>
    // quick&evil
    var grupaSelectorUrl = '<?php echo MainMenu::getModuleUrl('edit', 'grupa')?>'.replace(/&amp;/g, '&');
